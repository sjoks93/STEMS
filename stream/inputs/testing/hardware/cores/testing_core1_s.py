...
def get_memory_hierarchy(multiplier_array):
    ...
    reg_W = MemoryInstance(name="rf_W", size=4, r_bw=4, w_bw=4, r_cost=0.005, w_cost=0.005, area=0,
                             r_port=1, w_port=1, rw_port=0, latency=1)

    reg_V = MemoryInstance(name="rf_V", size=8, r_bw=8, w_bw=8, r_cost=0.01, w_cost=0.01, area=0,
                            r_port=2, w_port=2, rw_port=0, latency=1)
    reg_IO = MemoryInstance(name="rf_O", size=1, r_bw=1, w_bw=1, r_cost=0.00125, w_cost=0.00125, area=0,
                            r_port=2, w_port=2, rw_port=0, latency=1)
    ...
    '''
    memory_hierarchy_graph.add_memory(memory_instance=reg_IO, operands=('I1',),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': None, 'th': None},),
                                      served_dimensions={(1, 0)})
    memory_hierarchy_graph.add_memory(memory_instance=reg_W, operands=('I2',),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': None, 'th': None},),
                                      served_dimensions={(0, 0)})
    memory_hierarchy_graph.add_memory(memory_instance=reg_V, operands=('S',),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': 'w_port_2', 'th': 'r_port_2'},),
                                      served_dimensions={(0, 1)})
    memory_hierarchy_graph.add_memory(memory_instance=reg_IO, operands=('O',),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': 'w_port_2', 'th': 'r_port_2'},),
                                      served_dimensions={(0, 1)})
    ...
    memory_hierarchy_graph.add_memory(memory_instance=sram_32KB_with_4_8K_64_1r_1w, operands=('I1',),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': None, 'th': None},),
                                      served_dimensions='all')
    memory_hierarchy_graph.add_memory(memory_instance=sram_128K_with_1_128K_bank_128_1r_1w_A, operands=('I1', 'O', 'S'),
                                      port_alloc=({'fh': 'w_port_1', 'tl': 'r_port_1', 'fl': None, 'th': None},
...
def get_operational_array():
    ...
    multiplier_input_precision = [4, 1]
    multiplier_energy = 0.01
    multiplier_area = 1
