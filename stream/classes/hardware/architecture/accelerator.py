...
class Accelerator:
    ...
    def get_core(self, core_id: int or str) -> Core:
        ...
        if not core:
            core = next((core for core in self.cores.nodes() if core.id == core_id[0]), None)
            if not core:
                raise ValueError(
                    f"Requested core with id {core_id[0]} is not present in accelerator."
                )
        return core
    ...
    def remove(self, tensor, core, memory_op, timestep, write_back_to_offchip=False, is_state=False):  #TODO: need to add all costs here....
        ...
        if(is_state):
            should_be_written_to_offchip = True
        ...
        if should_be_written_to_offchip:
            ...
            assert len(eviction_link_energy_cost) == 0
            assert len(eviction_memory_energy_cost) == 0
    ...
    def remove_all(
        ...
        Args:
            ...
            write_back_to_offchip (bool, optional): Write the tensor to offchip before removal. Defaults to False.
        """
        total_link_energy_cost = []
        total_memory_energy_cost = []
        ...
        for tensor in self.memory_manager.get_tensors_stored_at_timestep(
            ...
            if not tensor in exceptions:
                ...
                is_state = (tensor.layer_operand == tensor.origin.state_operand)

                t, link_energy_cost, memory_energy_cost = self.remove(
                    tensor, core, memory_operand, t, write_back_to_offchip, is_state=is_state
                )
                # print(t, tensor)
    ...
    def make_space_for(
        ...
        for tensor_to_evict in tensors_to_evict:
            is_state = tensor_to_evict.layer_operand == tensor_to_evict.origin.state_operand
            (
                t_eviction_complete,
                eviction_link_energy_cost,
                eviction_memory_energy_cost,
            ...
            # if tensor_to_evict.layer_operand == tensor_to_evict.origin.weight_operand or tensor_to_evict.layer_operand == tensor_to_evict.origin.state_operand:
            #     print('Evict', tensor_to_evict, 'time', t_eviction_complete/1e6, 'cn end', tensor_to_evict.origin.end/1e6, enough_space_timestep/1e6)
    ...
    def transfer_tensor_to_core( #TODO: need to add all costs here....
        self,
        tensor: Tensor,
        receiving_core_id: int,
        tensor_operand: str,
        non_evictable_tensors: list,
        priority_dict: dict = None,
        sending_core_id: int = None,
        minimum_timestep: int = None,
    ):
        """
        ...
        Args:
            ...
            minimum_timestep (int, optional): Minimum timestep when core is ready to receive
        """
        ################################# STEP 0 #################################
        ...
        op = tensor.layer_operand
        output_operand = tensor.origin.output_operand
        state_operand = tensor.origin.state_operand
        state = tensor.origin.state
        ...
        if state:
            if state_operand == op:
                scale_access = tensor.origin.dram_access_scale
            else:
                scale_access = 1
        elif output_operand == op and self.offchip_core_id == receiving_core_id:
            scale_access = tensor.origin.dram_access_scale  
        else:
            scale_access = 1   
        ...
        if self.memory_manager.contains(tensor, receiving_top_instance):
            # For state tensors, if receiving target is offchip, transfer tensor anyway
            # if(tensor.origin.state and tensor.origin.state_operand == tensor.layer_operand and self.offchip_core_id == receiving_core_id):
            #     pass
            # else:            
            return -1, [], [], [], [], False
        ################################# STEP 1 #################################
        ...
        if sender_cores[0].id == receiving_core_id:
            return -1, [], [], [], [], False            
        ################################# STEP 2 #################################
        # The receiver core has enough space to store the tensor.
        if minimum_timestep is not None:
            available_since_timestep = max(available_since_timestep, minimum_timestep)
        enough_space_timestep = self.memory_manager.get_timestep_for_tensor_addition(
            tensor,
            receiving_core_id,
            available_since_timestep,
        ...
        transfer_duration = max([ceil(tensor.size * scale_access / link.bandwidth) for link in links])
        ...
        # if receiving_core_id != self.offchip_core_id and (op == tensor.origin.weight_operand or op == state_operand):
        #     print('To core', tensor, 'transfer end', transfer_end/1e6, 'eviction complete', evictions_complete_timestep/1e6 )	
        ...
        elif sender_core.id == self.offchip_core_id:
            pass
        else:
    ...
    def get_memory_energy_cost_of_transfer(
        ...
        nb_sender_memory_reads_for_data = ceil(
            tensor.size * scale_access / sender_top_memory_level.read_bw
        )
        sender_energy = (
        ...
        nb_receiver_memory_writes_for_data = ceil(
            ...
            tensor.size * scale_access / receiver_top_memory_level.write_bw
        )
        receiver_energy = (
        ...
        return [Cost(tensor=tensor, component=sender_top_memory_level.name, energy=sender_energy),
                Cost(tensor=tensor, component=receiver_top_memory_level.name, energy=receiver_energy)]
...
#     def remove(self, tensor, core, memory_op, timestep, cn_offchip_energy_breakdown, write_back_to_offchip=False, is_state=False):
#         """Remove tensor from core. If required, transfer to offchip before removal.
...
#                 memory_op,
#                 non_evictable_tensors=[],
#                 cn_offchip_energy_breakdown = cn_offchip_energy_breakdown,
#                 sending_core_id=core.id,
#                 minimum_timestep = minimum_timestep
#             )
#             cn_offchip_energy_breakdown.append({
#                 'layer_id': tensor.origin.id[0],
#                 'operand': tensor.layer_operand,
#                 'component': 'dram',
#                 'energy': transfer_link_energy_cost + transfer_memory_energy_cost
#             })
#             # if(tensor.layer_operand == "V"):   
#             #     print(cn_offchip_energy_breakdown[-1], transfer_end, "to dram", transfer_link_energy_cost, transfer_memory_energy_cost)            
#             # # There should be no evictions as we are writing to offchip
...
#     def remove_all(
#         self, core, memory_operand, timestep, cn_offchip_energy_breakdown, exceptions=[], write_back_to_offchip=False
#     ):
#         """Remove all tensors from a core's memory with the given memory operand.
#         If required, the tensors are written back to offchip before removal.
...
#             if not tensor in exceptions:
#                 t, link_energy_cost, memory_energy_cost = self.remove(
#                     tensor, core, memory_operand, t, cn_offchip_energy_breakdown, write_back_to_offchip
#                 )
#                 total_link_energy_cost += link_energy_cost
#                 total_memory_energy_cost += memory_energy_cost
...
#             non_evictable_tensors (list): the stored tensor that cannot be evicted
#             sending_core_id (int, optional): The id of the core that should transfer the tensor.
#             minimum_timestep (int, optional): Minimum timestep when updated tensor is available (for spiking states write-back)
#         """
#         ################################# STEP 0 #################################
#         # Check if the tensor is already on the receiving core
#         # Get the top instance where the tensor will be transferred to
...
#             receiving_core_id, tensor_operand
#         )
#         #print(self.memory_manager.contains(tensor, receiving_top_instance))
#         if self.memory_manager.contains(tensor, receiving_top_instance):
#             # For state tensors, if receiving target is offchip, transfer tensor anyway
#             if(tensor.origin.state and tensor.origin.state_operand == tensor.layer_operand and self.offchip_core_id == receiving_core_id):
#                 pass
...
#             assert self.contains_tensor(tensor, storing_instance)
#             if minimum_timestep is not None:
#                 available_since_timestep = minimum_timestep
#             else:
#                 available_since_timestep = (
#                     self.memory_manager.top_instance_available_since_timestep[
#                         storing_instance
