...
class ComputationNode(LayerNode, Node):
    ...
    def __init__(
        ...
        # output_tensor = self.operand_tensors[self.state_operand] if self.state else self.operand_tensors[self.output_operand]
        # if self.loop_dim_size.__contains__('C'):
        #     c = self.loop_dim_size['C']
        # else:
        #     c = 1
        # x_start, x_end = output_tensor.loop_ranges[output_tensor.loop_dimensions.index('OX')]
        # x = x_end - x_start
        # y_start, y_end = output_tensor.loop_ranges[output_tensor.loop_dimensions.index('OY')]
        # y = y_end - y_start            
        # if self.loop_dim_size.__contains__('T'):
        #     t = self.loop_dim_size['T']
        # else:
        #     t = 1
        # a, b = mapping_assist_funcs.count_zeros([t, c, y, x, self.filter_size[0], self.filter_size[1]], self)
        self.dram_access_scale = 1
    ...
    def __hash__(self) -> int:
        ...
        """
        id = list(copy.deepcopy(self.id))
        if (len(id) > 1):
            idx_new = 0
            for i, val in enumerate(id[1]):
                idx_new = idx_new + val * pow(100, 1-i)
            id[1] = idx_new
        id = tuple(id)
        return hash(id)
