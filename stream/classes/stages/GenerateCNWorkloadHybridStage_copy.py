...
class GenerateCNWorkloadHybridStage(Stage):
    ...
    def run(self):
        ...
        for node in nx.topological_sort(self.workload):
            ...
            outer_temporal_loops = self.get_outer_tmap_loop_dimensions(node, block_id)
    ...
    def get_outer_tmap_loop_dimensions(self, layer, block_id = None) -> List[TemporalLoop]:
        ...
        for node in nx.topological_sort(G):
            ...
            for layer_operand in node.operand_list:
                ...
                if layer_operand == node.output_operand:
                    ...
                    if tensor.id[0] == 4 and tensor.id[1] == 17 and tensor.id[2] == 'O':
