...
def sorter(node):
    keys = ()
    for i in range(len(node.id[1])):
        #key = node.id[1][len(node.id[1])-1-i]
        key = node.id[1][i]
        keys = keys + (key,)
...
class GenerateCNWorkloadHybridStage(Stage):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fuse_loops = fuse_loops
        if len(self.hint_loops) == 0:
            self.list_loops = False
        elif isinstance(self.hint_loops[0], list):
            self.list_loops = True
        else:
            self.list_loops = False
    ...
    def run(self):
        ...
        for node in nx.topological_sort(self.workload):
            ...
            finer_nodes, unique_nodes = self.get_finer_nodes(node, outer_temporal_loops, self.fuse_loops)
    ...
    @staticmethod
    def get_finer_nodes(
        original_node, outer_temporal_loops, fuse_loops
        ...
        block_id = original_node.cut_block
        ...
        for n in range(nb_cns):
            ...
            finer_node = ComputationNode(
                (original_node_id, outer_loop_values),
            ...
            if finer_node.state:
                state_operand = original_node.state_operand
                tensor = finer_node.operand_tensors[state_operand]
                tensor.set_base_priorities(tensor_reuse_factors[state_operand][n])
            ...
            if(finer_node.state):               
                finer_node.data_produced_unique = (
                    ...
                    * finer_node.operand_precision["O"]
                )
            else:
                finer_node.data_produced_unique = (
                    finer_node.operand_size_elem["O"]
    ...
    def get_intra_edges(nodes):
        ...
        # group_ids = sorted(set([n.group for n in nodes]))
        # cut_ids = []
        # for index in range(len(nodes[0].id[1])):
        #     cut_ids.append(sorted(set([n.id[1][index] for n in nodes])))
        # intra_edges = []
        # for group_id in group_ids:
        #     group_nodes = [n for n in nodes if n.group == group_id]
        #     for index, cut_ids_list in enumerate(cut_ids):
        #         for cut_id in cut_ids_list:
        #             cut_nodes = [n for n in group_nodes if n.id[1][index] == cut_id]
        #             pairs = zip(cut_nodes, cut_nodes[1:])
        #             for node_1, node_2 in pairs:
        #                 intra_edges.append((node_1, node_2, {"bits": 0}))
        # return intra_edges
    ...
    def set_base_priority_of_nodes(G, finer_nodes_dict):
        ...
        for node in G.nodes:#nx.topological_sort(G):
            layer_id = node.id[0]
...
def deduce_tensor_reuse_factors(original_node, outer_temporal_loops) -> dict[list[int]]:
    ...
    for op, total_reuse_factor in total_reuse_factors.items():
        tensor_reuse_factors[op] = [total_reuse_factor] * int(nb_nodes)
