...
class IntraCoreMappingStage(Stage):
    ...
    def run(self):
        ...
        for node in self.unique_nodes:
            ...
            for core_id in self.valid_allocations[node]:
                ...
                try:
                    print(self.given_node_hw_performances[node][core])
                except:
                    print("exception")
                ...
                print(self.node_hw_performances[node][core].mapping.output_tile_rate)
                # if(self.node_hw_performances[node][core].mapping.layer_node.state):
                #     print(self.node_hw_performances[node][core].mapping.combined_mapping_dict_1s1t_reform['V'])
                # else:
                print(self.node_hw_performances[node][core].mapping.combined_mapping_dict_1s1t_reform['O'])
                #print(self.node_hw_performances[node][core].mapping.combined_mapping_dict_1s1t_reform['W'])
    ...
    def check_core_capacity_for_node(self, core: Core, node: ComputationNode):
        ...
        constant_operands = node.constant_operands.copy()
        output_operand = node.output_operand
        if(node.state):
            constant_operands.append(node.state_operand)
        for top_memory in unique_top_memories:
            ...
            layer_operands = [
                layer_operand_link[mem_operand] for mem_operand in memory_operands if layer_operand_link.__contains__(mem_operand)
            ]
