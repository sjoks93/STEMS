...
class MemoryManager:
    ...
    def add_tensor_to_core(
        ...
        if (
           ...
           # raise ValueError(
            print(f"Inserting {tensor} in {top_instance} caused memory overflow.")
           # )
    ...
    def find_best_tensor_combination_to_evict_fast(
        ...
        for tensor in evictable_tensors:
            if priority_dict is not None and (tensor.layer_operand == tensor.origin.weight_operand or tensor.layer_operand == tensor.origin.state_operand):
                instance_priority = priority_dict[tensor.equality_hash()]
            else:
                instance_priority = tensor.get_instance_priority(top_instance, self)
            importance = instance_priority * tensor.size
            importance = (instance_priority *  100) - tensor.id[0]
            evictable_tensors_priority_size.append(importance)

        else:
            if len(evictable_tensors) == 0:
                print(tensor_to_add.size, capacity, stored_tensors_size, stored_tensors, 'add', tensor_to_add)
    ...
    def remove_tensor_from_top_instance(
        ...
        try:
            ...
            return
        if top_instance.name == "dram" and tensor.id[2] != "V":
            return
