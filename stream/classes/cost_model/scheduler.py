...
def initialize_priorities(G: DiGraph, accelerator: Accelerator):
    ...
    for n in G.nodes():
        for op, tensor in n.operand_tensors.items():
            ...
            if op == tensor.origin.weight_operand or op == tensor.origin.state_operand:
                priority_dict[tensor.equality_hash()] = tensor.base_priority
    return priority_dict

#priority_dict[equality_hash] =
...
def initialize_offchip_tensors(G: DiGraph, accelerator: Accelerator):
    ...
    for n in G.nodes():
        for op, tensor in n.operand_tensors.items():
            if op in n.constant_operands or (op == n.state_operand):
...
def prefetch_constant_operands(G: DiGraph, accelerator: Accelerator, operands_to_prefetch: list):
    ...
    for n in G.nodes():
        for op, tensor in n.operand_tensors.items():
            if (op in n.constant_operands or op == n.state_operand) and op in operands_to_prefetch:
...
def get_best_candidate(candidates: list, candidate_selection: str):
    ...
    elif candidate_selection == "LYT_gated":
        candidate_ids = [(-cn.fuse_id ,-cn.id[1][1], -cn.id[1][0], cn.id[0], -cn.group) for cn in cn_candidates]
        best_candidate_idx = candidate_ids.index(max(candidate_ids))
        best_candidate = cn_candidates[best_candidate_idx]
        preds_end = preds_ends[best_candidate_idx]     
    else:
...
def clear_memories(accelerator: Accelerator, core: Core, memory_operands, timestep, exceptions=None):
    # total_eviction_to_offchip_link_energy = 0
    # total_eviction_to_offchip_memory_energy = 0
    total_eviction_to_offchip_link_energy = []
    total_eviction_to_offchip_memory_energy = []
...
def decrease_priority(
    ...
    for tensor_used_by_node, tensor_memory_operand in zip(tensors, tensors_operands):
        ...
        if tensor_used_by_node.layer_operand == tensor_used_by_node.origin.state_operand or tensor_used_by_node.layer_operand == tensor_used_by_node.origin.weight_operand:
            priority_dict[tensor_used_by_node.equality_hash()] -= 1
        else:
...
def check_for_removal(
    ...
    for tensor_used_by_node in tensors:
        ...
        # print(tensor_used_by_node, tensor_used_by_node.get_total_priority())
        if tensor_used_by_node.layer_operand == tensor_used_by_node.origin.weight_operand or tensor_used_by_node.layer_operand == tensor_used_by_node.origin.state_operand:
            prio = priority_dict[tensor_used_by_node.equality_hash()]
            #print(tensor_used_by_node, prio)
        else:
            prio = tensor_used_by_node.get_total_priority()
        ...
        if prio == 0:
            ...
            for instance_storing_tensor in instances_storing_tensor:
               ...
               # print(tensor_used_by_node)
                #print('remove', tensor_used_by_node, timestep_for_removal/1e6)
                if(tensor_used_by_node.layer_operand == tensor_used_by_node.origin.state_operand and offchip_core_id != core_ids_of_instance[0]):
                    is_state = True
                else:
                    is_state = False
                if core.id != offchip_core_id or tensor_used_by_node.layer_operand == tensor_used_by_node.origin.state_operand:
                    timestep_for_removal, link_energy_cost_n, memory_energy_cost_n = accelerator.remove(
                        tensor_used_by_node,
                        core,
                    ...
                    link_energy_cost += link_energy_cost_n
                    memory_energy_cost += memory_energy_cost_n
    return timestep_for_removal, link_energy_cost, memory_energy_cost
...
def schedule_graph(
    ...
    priority_dict = initialize_priorities(G, accelerator)
    ...
    while not done:
        ...
        # print('Start', best_candidate)
        # for tensor in tensors_this_candidate_needs:
        #     print(tensor, tensor.equality_hash())
        # print('end')
        # print(best_candidate, best_candidate.id, best_candidate.output_tile_rate)
        ## Step 1
        ...
        for tensor, tensor_operand in zip(
            ...
            (
                ...
                minimum_timestep = cores_idle_from[core_id]
            )
            # Update the possible start time of this node
            timestep = max(timestep, transfer_complete_timestep)
           
            # if tensor is the predecessor of the node AND the layer_operand is O, then clone tensor and change operand to I and id to current node id.
            if tensor.layer_operand == 'O':
                layer_operand = None
                for op in ['I', 'X', 'Y']:
                    try:
                        group = best_candidate.input_operand_source[op].group
                    except KeyError:
                        continue
                    if group == tensor.origin.group:
                        layer_operand = op
                        break   
                if layer_operand is None:    
                    layer_operand = 'I'
                new_tensor = Tensor(size=tensor.size, origin=best_candidate, layer_operand=layer_operand)

                transfer_link_energy_cost = [Cost(tensor=new_tensor, component=cost.component, energy=cost.energy) for cost in transfer_link_energy_cost]
                transfer_memory_energy_cost = [Cost(tensor=new_tensor, component=cost.component, energy=cost.energy) for cost in transfer_memory_energy_cost]            
        ...
        decrease_priority(
            tensors_this_candidate_needs, tensors_operands, accelerator, best_candidate, priority_dict
        )
        # Remove the tensor if the priority is zero
        endo, link_cost, memory_cost = check_for_removal(
            tensors_this_candidate_needs,
            accelerator,
            best_candidate,
            G,
        ...
        total_cn_offchip_link_energy += link_cost
        total_cn_offchip_memory_energy += memory_cost
        ...
        #print(best_candidate, start/1e6, end/1e6)
        ...
        # # Remove the tensor if the priority is zero
        # check_for_removal(tensors_this_candidate_needs, accelerator, best_candidate, G, end, )
        ...
        for successor in sorted(G.successors(best_candidate)):
            ...
            #else:
            #    print('not added')
            #    print('predecesors', [pred for pred in G.predecessors(successor)])
            #    print('scheduled', scheduled_nodes)

        # Increment the number of scheduled nodes
        nb_scheduled_nodes += 1
        #print(nb_scheduled_nodes, '/', nb_graph_nodes, candidates)
    ...
    energy_breakdown = {
        "cn_onchip_energy": total_cn_onchip_energy,
        "cn_offchip_link_energy": total_cn_offchip_link_energy,
        "cn_offchip_memory_energy": total_cn_offchip_memory_energy,
        "eviction_to_offchip_link_energy": total_eviction_to_offchip_link_energy,
        "eviction_to_offchip_memory_energy": total_eviction_to_offchip_memory_energy,
        "sink_layer_output_offchip_link_energy": total_sink_layer_output_offchip_link_energy,
        "sink_layer_output_offchip_memory_energy": total_sink_layer_output_offchip_memory_energy,
        "core_to_core_link_energy": total_core_to_core_link_energy,
        "core_to_core_memory_energy": total_core_to_core_memory_energy,
    }
    return (latency, energy_breakdown)
####################################################
...
#     for n in G.nodes():
#         for op, tensor in n.operand_tensors.items():
#             if op in n.constant_operands or (n.state and op == n.state_operand):
#                 if not any(
#                     (
#                         accelerator.contains_tensor(tensor, offchip_top_instance)
#                         for offchip_top_instance in offchip_top_instances
...
#     G: DiGraph, accelerator: Accelerator, operands_to_prefetch: list, cn_offchip_energy_breakdown
# ):
#     total_cn_offchip_link_energy = 0
#     total_cn_offchip_memory_energy = 0
#     total_eviction_to_offchip_link_energy = 0
...
#                         came_from_offchip,
#                     ) = accelerator.transfer_tensor_to_core(
#                         tensor, core_allocation, memory_op, [], cn_offchip_energy_breakdown
#                     )
#                     cn_offchip_energy_breakdown.append({
#                         'layer_id': n.id[0],
#                         'operand': op,
#                         'component': 'dram',
#                         'energy': transfer_link_energy_cost + transfer_memory_energy_cost
#                     })   
#                     #print(cn_offchip_energy_breakdown[-1], transfer_complete_timestep, "init to core")                   
#                     assert came_from_offchip
#                     total_cn_offchip_link_energy += transfer_link_energy_cost
#                     total_cn_offchip_memory_energy += transfer_memory_energy_cost
#                     total_eviction_to_offchip_link_energy += eviction_link_energy_cost
...
# def clear_memories(
#     accelerator: Accelerator, core: Core, memory_operands, timestep, cn_offchip_energy_breakdown, exceptions=[]
# ):
#     total_eviction_to_offchip_link_energy = 0
#     total_eviction_to_offchip_memory_energy = 0
#     for too_large_operand in memory_operands:
...
#             eviction_memory_energy_cost,
#         ) = accelerator.remove_all(
#             core, too_large_operand, timestep, cn_offchip_energy_breakdown, exceptions, write_back_to_offchip=True
#         )
#         total_eviction_to_offchip_link_energy += eviction_link_energy_cost
#         total_eviction_to_offchip_memory_energy += eviction_memory_energy_cost
#     return (
...
#                     instance_storing_tensor
#                 ][0]
#                 if( tensor_used_by_node.origin.state and tensor_used_by_node.layer_operand == tensor_used_by_node.origin.state_operand and offchip_core_id != core_ids_of_instance[0]):
#                     is_state = True
#                 else:
#                     is_state = False
#                 timestep_for_removal, link_energy_cost_n, memory_energy_cost_n = accelerator.remove(
...
#         prefetch_eviction_to_offchip_link_energy,
#         prefetch_eviction_to_offchip_memory_energy,
#     ) = prefetch_constant_operands(G, accelerator, operands_to_prefetch, cn_offchip_energy_breakdown)
#     total_cn_offchip_link_energy += prefetch_cn_offchip_link_energy
#     total_cn_offchip_memory_energy += prefetch_cn_offchip_memory_energy
#     total_eviction_to_offchip_link_energy += prefetch_eviction_to_offchip_link_energy
#     total_eviction_to_offchip_memory_energy += (
...
#             # Add the energy costs to their respective trackers
#             if came_from_offchip:
#                 # node id, operand, component, energy
#                 cn_offchip_energy_breakdown.append({
#                     'layer_id': best_candidate.id[0],
#                     'operand': tensor.layer_operand,
#                     'component': 'dram',
#                     'energy': transfer_link_energy_cost + transfer_memory_energy_cost
#                 })       
#                 # print(cn_offchip_energy_breakdown[-1], timestep, "inputs to core")         
#                 total_cn_offchip_link_energy += transfer_link_energy_cost
#                 total_cn_offchip_memory_energy += transfer_memory_energy_cost
#             else:
#                 total_core_to_core_link_energy += transfer_link_energy_cost
...
#                 total_sink_layer_output_offchip_link_energy += link_energy_cost
#                 total_sink_layer_output_offchip_memory_energy += memory_energy_cost
#                 # Output sink layer to breakdown
#                 #memory_operand_links_inverted = {v: k for k, v in best_candidate.memory_operand_links.items()}
#                 # node id, operand, component, energy
#                 # cn_offchip_energy_breakdown.append({
#                 #     'node_id': best_candidate.id[0],
#                 #     'operand': output_tensor.layer_operand,
#                 #     'component': 'dram',
#                 #     'energy': link_energy_cost + memory_energy_cost
#                 # })
#                 #print(cn_offchip_energy_breakdown[-1], current_timestep, "final to dram")                
...
#     # print("Scheduling completed")
#     # print(f"Latency found = {latency}")
#     # for offchip_energy in cn_offchip_energy_breakdown:
#     #     print(offchip_energy)
#     return (
#         latency,
#         total_cn_onchip_energy,
#         total_cn_offchip_link_energy,
