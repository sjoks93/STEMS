...
class LomaEngine:
    ...
    def run(self):
        ...
        for ordering in self.og():  # ordering generator
            if self.layer.time_operand is not None and self.layer.state:
                found = False
                first_time_index = 100
                last_ir_index = 0
                relevancy_table = self.layer.operand_loop_dim_reform[self.layer.output_operand]
                for index, factor in enumerate(ordering):
                    if not found and factor[0] == self.layer.time_operand:
                        found = True
                        first_time_index = index
                    if relevancy_table["ir"].__contains__(factor[0]):
                        last_ir_index = index
