...
class MemoryAllocator:
    ...
    def __init__(self, accelerator, layer, spatial_mapping, ordering: List):
        ...
        if(self.layer.state):
            ...
            self.precision = {
                mem_op: self.layer.operand_precision[layer_op]
                for (layer_op, mem_op) in self.layer_to_mem_op.items() if self.layer.operand_precision.__contains__(layer_op)
            }
            self.precision[self.layer.output_operand + "_final"] = self.layer.operand_precision.get(
                self.layer.output_operand + "_final", self.precision["O"]
    ...
    def allocate_node(self, node: MemoryLevel, top_levels: List[MemoryLevel]):
        ...
        for mem_op in mem_ops:
            sizes = self.calc_size_slices(mem_op, mem_capacity, node)
    ...
    def calc_size_slices(self, mem_op: str, mem_capacity: int, node):
        ...
        for i in range(
            ...
            if size <= mem_capacity:
                ...
                if i == 0:  # This means we can't even store the already allocated loops
                    #print(f"Memory capacity overflow for mem_op {mem_op}. loops={loops} size={size} mem_capacity={mem_capacity}-{node.name}")
    ...
    def calc_loops_size(self, loops: List, mem_op: str, all_unallocated_loops: List):
        ...
        if self.layer.state == False and self.mem_to_layer_op[mem_op] == self.layer.output_operand:
            ...
            if any([dim in ir_dims for dim in unallocated_dims]):
                precision = self.precision[self.layer.output_operand]
            else:
                precision = self.precision[self.layer.output_operand+"_final"]
        else:
