...
class PortActivity:
    ...
    def __init__(
        ...
        """ Total required DTL transfer time """
        self.X_real = real_cycle * (period_count - 1)
        """ total memory updating window allowed """
        self.MUW = allowed_cycle * (period_count - 1)
        self.associated_S = None
    def __str__(self):
    ...
    def is_Associated(self):
        return self.associated_S is not None
    def associate_s_activity(self, port_activity_S):
        self.associated_S = port_activity_S
...
class CostModelEvaluation:
        ...
        return {
            "outputs": {
                ...
                "energy": {
                    ...
                    "output_tile_rate": [str(tile) for tile in self.mapping.output_tile_rate],
        ...
        for mem_share_dict in self.mem_sharing_list:
            ...
            for mem_op, mem_lv in mem_share_dict.items():
                try:
                    ...
                    continue
                if(mem_utili_individual.__contains__(layer_op)):
            ...
            for mem_op, mem_lv in mem_share_dict.items():
                try:
                    ...
                    continue
                if(mem_utili_shared.__contains__(layer_op)):
    ...
    def calc_memory_word_access(self):
        ...
        output_size = self.layer.operand_size_elem[output_operand]
        mem_bw_boost_factor = self.spatial_mapping.mem_bw_boost[output_operand] + [1]    
        ...
        for layer_op in self.layer.operand_list:
            ...
            for mem_lv in range(self.mapping.mem_level[layer_op]):
                sparsity_support = memory_levels[mem_lv].memory_instance.support_sparsity

                if(mem_lv < self.mapping.mem_level[layer_op] - 1):
                    sparsity_support_higher = memory_levels[mem_lv+1].memory_instance.support_sparsity  
                else:
                    sparsity_support_higher = False
                ...
                if sparsity_support:
                    access_scale = self.layer.i_rate
                elif layer_op == self.layer.state_operand:
                    access_scale = self.mapping.output_tile_rate[mem_lv]
                elif layer_op == self.layer.output_operand and not self.layer.state:
                    access_scale = self.mapping.output_tile_rate[mem_lv]
                else:
                    access_scale = 1
                ...
                if data_elem_move_per_period == 0 or data_precision == 0:
                    ...
                    wr_in_by_low_da = self.mapping.unit_mem_data_movement[layer_op][mem_lv].data_trans_amount_per_period.wr_in_by_low 
                    if(layer_op == output_operand and self.layer.state and wr_in_by_low_da != 0):
                        total_period_count = output_size/(wr_in_by_low_da * mem_bw_boost_factor[mem_lv+1])
                    else:                    
                ...
                if sparsity_support_higher:
                    access_scale = self.layer.i_rate
                elif layer_op == self.layer.state_operand:
                    access_scale = self.mapping.output_tile_rate[mem_lv+1]
                elif layer_op == self.layer.output_operand and not self.layer.state:
                    access_scale = self.mapping.output_tile_rate[mem_lv+1]
                else:
                    access_scale = 1              
                if data_elem_move_per_period == 0:
                    ...
                    rd_out_to_high_da = self.mapping.unit_mem_data_movement[layer_op][mem_lv].data_trans_amount_per_period.rd_out_to_high  
                    if(layer_op == output_operand and self.layer.state and rd_out_to_high_da != 0):             
                        total_period_count = output_size/(rd_out_to_high_da * mem_bw_boost_factor[mem_lv+1])
                    else:                    
                        total_period_count = self.mapping.unit_mem_data_movement[layer_op][
                            mem_lv
    ...
    def calc_MAC_energy_cost(self):
        ...
        if(core.sparse_mac):
            self.MAC_energy *= self.layer.i_rate
    ...
    def calc_allowed_and_real_data_transfer_cycle_per_DTL(self):
        ...
        for layer_op in self.layer.operand_list:
            ...
            if(self.layer.state and self.layer.output_operand == layer_op):
                real_data_trans_cycle_S = []
            ...
            for mem_lv in range(self.mapping_int.mem_level[layer_op]):
                ...
                if(self.layer.state and self.layer.output_operand == layer_op):
                    if(self.mapping.psum_flag[mem_lv] == False):
                        wr_in_by_low_real = 0
                    if(self.mapping.psum_flag[mem_lv+1] == False):
                        rd_out_to_high_real = 0
                    real_data_trans_S = FourWayDataMoving(
                    rd_out_to_low_real,
                    wr_in_by_low_real,
                    rd_out_to_high_real,
                    wr_in_by_high_real,
                )
                    real_data_trans_cycle_S.append(real_data_trans_S)
    ...
    # Step 2: calculate SS combine and MUW union parameters per physical memory port
        if self.layer.state:
            self.real_data_trans_cycle_S = real_data_trans_cycle_S
    def combine_data_transfer_rate_per_physical_port(self):
            ...
            for port in port_list:
                ...
                for mem_op, mem_lv, mov_dir in port.served_op_lv_dir:
                    try:
                        ...
                        continue
                    if(self.mapping_int.unit_mem_data_movement.__contains__(layer_op)):
                        ...
                        if self.layer.state and layer_op == self.layer.output_operand:
                            real_cycle_S = getattr(self.real_data_trans_cycle_S[mem_lv], mov_dir)
                            flag = True
                        else:
                            flag = False
                        ...
                        if layer_op == self.layer.output_operand or layer_op == self.layer.state_operand:
                            if mov_dir == "rd_out_to_low" or mov_dir == "wr_in_by_low":
                                period_count = period_count * self.mapping.output_tile_rate[mem_lv]
                            else:
                                period_count = period_count * self.mapping.output_tile_rate[mem_lv+1]
                        ...
                        if(flag):
                            ...
                            port_activity.associate_s_activity(port_activity_S)
        ...
        for idx, mem_ports in enumerate(port_activity_collect):
            for port_name, port_activity in mem_ports.items():
                if len(port_activity) == 1:
                    ...
                    X_real.append(port_activity[0].X_real)
                    if port_activity[0].is_Associated():
                        SS_comb_list_S.append(port_activity[0].associated_S.SS)
                    elif self.layer.state:
                        SS_comb_list_S.append(port_activity[0].SS)
                    ...
                    for port_d in port_activity:
                        X_real_sum += port_d.X_real
                        if port_d.SS > 0:
                            SS_positive_sum += port_d.SS
                            if self.layer.state:
                                if port_d.is_Associated():
                                    SS_positive_sum_S += port_d.associated_S.SS
                                else:
                                    SS_positive_sum_S += port_d.SS
                        else:
                            SS_negative_sum += port_d.SS
                            if self.layer.state:
                                if port_d.is_Associated():
                                    SS_negative_sum_S += port_d.associated_S.SS
                                else:
                                    SS_negative_sum_S += port_d.SS
                        if self.layer.state:
                            if port_d.is_Associated():
                                MUW_sum_S += port_d.associated_S.MUW
                            else:
                                MUW_sum_S += port_d.MUW
                        MUW_sum += port_d.MUW
                    SS_comb = SS_positive_sum + max(
                        0, SS_negative_sum + MUW_sum - MUW_union_collect[idx][port_name]
                    )
                    if self.layer.state:
                        SS_comb_S = SS_positive_sum_S + max(
                            0, SS_negative_sum_S + MUW_sum_S - MUW_union_collect[idx][port_name]
                        )                    
                    SS_comb_collect[idx][port_name] = SS_comb
                    SS_comb_list.append(SS_comb)
                    X_real.append(X_real_sum)
                    if self.layer.state:
                        SS_comb_list_S.append(SS_comb_S)
        ...
        self.x_real = max(X_real)
        if self.layer.state:
            self.SS_comb_S = max(SS_comb_list_S)
            self.SS_comb = (self.SS_comb + self.SS_comb_S * (self.temporal_mapping.top_ir_prod - 1) )/self.temporal_mapping.top_ir_prod
    ...
    def calc_data_loading_offloading_latency(self):
        ...
        for mem_inst_idx, mem_instance in enumerate(self.mem_level_list):
            ...
            for port in port_list:
                ...
                served_operands = set(
                    s[0] for s in port.served_op_lv_dir if s[0] in ["I1", "I2", "S"]
                )
                served_outputs = set( s[0] for s in port.served_op_lv_dir if s[0] in ["S", "O"])
                port_is_shared_by_multiple_input_operands = len(served_operands) > 1
                port_is_shared_by_multiple_output_operands = len(served_outputs) > 1
                for mem_op, mem_lv, mov_dir in port.served_op_lv_dir:
                    try:
                        layer_op = self.mem_op_to_layer_op[mem_op]
                    except:  # mem op to layer op might not have this mem op (e.g. pooling layer)
                    ...
                    if(self.mapping_int.unit_mem_data_movement.__contains__(layer_op)):
                        ...
                        if mem_op in ["I1", "I2", "S"]:
                            ...
                            if mov_dir in ["rd_out_to_low", "wr_in_by_high"]:
                                ...
                                ] = (real_cycle, port_is_shared_by_multiple_input_operands)
                        if mem_op in ["O", "S"]:
                            if mov_dir in ["rd_out_to_high", "wr_in_by_low"]:
                                # don't consider partial sum flowing in the final data off-loading stage
                                real_cycle = getattr(
                                    self.real_data_trans_cycle[layer_op][mem_lv], mov_dir
                                )
                                ...
                                data_offloading_cc_per_op[layer_op][
                                    layer_op + str(mem_lv) + "_" + mov_dir
                                ] = (real_cycle, port_is_shared_by_multiple_output_operands)
        ...
        list = self.layer.input_operands
        if(self.layer.state):
            data_loading_cc_pair_combined_per_op[self.layer.state_operand] = []
            data_loading_individual_part[self.layer.state_operand] = 0
            data_loading_half_shared_part[self.layer.state_operand] = 0
            data_loading_shared_part[self.layer.state_operand] = 0        
            list.append(self.layer.state_operand)
        for layer_op in list:
        ...
        if len(list) == 1:
            ...
            possible1 = data_loading_shared_part[op1] + max(
                ...
                + data_loading_half_shared_part[op2]
                + data_loading_individual_part[op2]
                + data_loading_shared_part[op3]
                + data_loading_half_shared_part[op3]
                + data_loading_individual_part[op3],
                data_loading_half_shared_part[op1] + data_loading_individual_part[op1],
            )
            possible2 = data_loading_shared_part[op2] + max(
                data_loading_shared_part[op1]
            ...
            possible3 = data_loading_shared_part[op3] + max(
                data_loading_shared_part[op1]
                + data_loading_half_shared_part[op1]
                + data_loading_individual_part[op1]
                + data_loading_shared_part[op2]
                + data_loading_half_shared_part[op2]
                + data_loading_individual_part[op2],
                data_loading_half_shared_part[op3] + data_loading_individual_part[op3],
            )            
            data_loading_cycle = min(possible1, possible2, possible3)            
        ...
        data_offloading_cc_pair_combined_per_op = {}
        data_offloading_cc_pair_combined_per_op[self.layer.output_operand] = []
        data_offloading_individual_part = {}
        data_offloading_individual_part[self.layer.output_operand] = 0
        data_offloading_half_shared_part = {}
        data_offloading_half_shared_part[self.layer.output_operand]= 0
        data_offloading_shared_part = {}
        data_offloading_shared_part [self.layer.output_operand] = 0
        layer_op_l = [self.layer.output_operand]
        if(self.layer.state):
            data_offloading_cc_pair_combined_per_op[self.layer.state_operand] = []
            data_offloading_individual_part[self.layer.state_operand] = 0
            data_offloading_half_shared_part[self.layer.state_operand] = 0
            data_offloading_shared_part[self.layer.state_operand] = 0                
            layer_op_l.append(self.layer.state_operand)
        for layer_op in layer_op_l:
            for mem_lv in range(self.active_mem_level[layer_op] - 1):
                elem1 = data_offloading_cc_per_op[layer_op][
                    layer_op + str(mem_lv) + "_" + "rd_out_to_high"
                ]
                elem2 = data_offloading_cc_per_op[layer_op][
                    layer_op + str(mem_lv + 1) + "_" + "wr_in_by_low"
                ]
                completely_shared = elem1[1] and elem2[1]
                completely_separate = not (elem1[1]) and not (elem2[1])
                longest_offloading_cc = max(elem1[0], elem2[0])
                # for the ports that serve the same data movement purpose, take the longest data loading cycle
                data_offloading_cc_pair_combined = longest_offloading_cc
                data_offloading_cc_pair_combined_per_op[layer_op].append(
                    data_offloading_cc_pair_combined
                )
                if completely_separate:
                    data_offloading_individual_part[layer_op] += longest_offloading_cc
                elif completely_shared:
                    data_offloading_shared_part[layer_op] += longest_offloading_cc
                else:
                    # the data transfer link between two memory levels is half-shared,
                    # i.e. on one memory side, the port is shared, while on another memory side,
                    # there are different memories with separate ports
                    data_offloading_half_shared_part[layer_op] = longest_offloading_cc
        if len(layer_op_l) == 1:
            data_offloading_cycle = data_offloading_individual_part[
                layer_op_l[0]
            ]
        else:
            op1 = layer_op_l[0]
            op2 = layer_op_l[1]
            possible1 = data_offloading_shared_part[op1] + max(
                data_offloading_shared_part[op2]
                + data_offloading_half_shared_part[op2]
                + data_offloading_individual_part[op2],
                data_offloading_half_shared_part[op1] + data_offloading_individual_part[op1],
            )
            possible2 = data_offloading_shared_part[op2] + max(
                data_offloading_shared_part[op1]
                + data_offloading_half_shared_part[op1]
                + data_offloading_individual_part[op1],
                data_offloading_half_shared_part[op2] + data_offloading_individual_part[op2],
            )
            data_offloading_cycle = min(possible1, possible2)   


        self.data_offloading_cc_pair_combined_per_op = data_offloading_cc_pair_combined_per_op
        self.data_offloading_individual_part = data_offloading_individual_part
        self.data_offloading_half_shared_part = data_offloading_half_shared_part
        self.data_offloading_shared_part = data_offloading_shared_part
    ...
    def calc_overall_latency(self):
        ...
        core = self.accelerator.get_core(self.core_id)
        if core.sparse_speedup:
            speed_up = self.layer.i_rate
        else:
            speed_up = 1
        ideal_cycle *= speed_up
        ideal_temporal_cycle *= speed_up
        ...
        if core.sparse_speedup:
            latency_total0 = max(latency_total0, self.x_real)
        ...
        latency_total1 = latency_total0 + self.data_loading_cycle
        # latency_total1_S = ideal_temporal_cycle + self.SS_comb_S + self.data_loading_cycle
        MAC_utilization1 = ideal_cycle / latency_total1
        # MAC_utilization1_S = ideal_cycle / latency_total1_S
