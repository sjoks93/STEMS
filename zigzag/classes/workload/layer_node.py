...
class LayerNode:
    ...
    def __init__(self, layer_id, layer_attrs, node_name="", type=None):
        ...
        #sparse: bool = layer_attrs.get("sparse", False)
        i_rate: float = layer_attrs.get("i_rate", 1)
        ...
        input_path: str = layer_attrs.get("input_path", None)
        ...
        weight_operand: str = layer_attrs.get("weight_operand", "W")
        time_operand: str = layer_attrs.get("time_operand", None)
        cut_block: int = layer_attrs.get("cut_block", 0)
        ...
        if(self.loop_dim_size.__contains__('FX') and self.loop_dim_size.__contains__('FY')):
            self.filter_size = [self.loop_dim_size['FX'], self.loop_dim_size['FY']]
        else:
            self.filter_size = [1, 1]
        self.weight_operand = weight_operand
        ...
        if input_path is not None:
            self.input_tile = np.load(input_path)
            if time_operand is None:
                self.input_tile = np.expand_dims(self.input_tile, axis=0)
            self.input_tile_history = []
           # self.sparse = True
            self.i_rate = mapping_assist_funcs.get_input_rate(self.input_tile)    
            #print(self.i_rate)   
        if(state):
            state_operand: Dict[str, list] = layer_attrs.get("state_operand", "V")       
        else:
            state_operand = None
        # Step1: extract partially-relevant data dimension and its relation to loop dimensions.
        pr_loop, pr_loop_list, pr_scaling_factors, stride = self.build_pr_funcs()
        ...
        self.time_operand = time_operand
        #print(state_operand_source.keys().)
        if(self.state):
            self.operand_list.append(state_operand)
            self.state_operand = state_operand
            self.operand_loop_dim[self.state_operand] = copy.deepcopy(self.operand_loop_dim[self.output_operand])
            self.operand_loop_dim_reform[self.state_operand] = copy.deepcopy(self.operand_loop_dim_reform[self.output_operand])
            self.operand_dimensionality_order[self.state_operand] = copy.deepcopy(self.operand_dimensionality_order[self.output_operand])
            if time_operand is not None:
                if self.operand_loop_dim[self.state_operand]['r'].__contains__(time_operand):
                    self.operand_loop_dim[self.state_operand]['r'].remove(time_operand)
                    self.operand_loop_dim[self.state_operand]['ir'].append(time_operand)
                if self.operand_loop_dim_reform[self.state_operand]['r'].__contains__(time_operand):
                    self.operand_loop_dim_reform[self.state_operand]['r'].remove(time_operand)
                    self.operand_loop_dim_reform[self.state_operand]['ir'].append(time_operand)
                if self.operand_dimensionality_order[self.state_operand].__contains__(time_operand):
                    self.operand_dimensionality_order[self.state_operand].remove(time_operand)
    ...
    def build_pr_funcs(self):
        ...
        if self.dimension_relations:
            pr_loop, pr_loop_list, pr_scaling_factors, stride = self.extract_pr_loop_info(
                self.dimension_relations
            )
        else:
            pr_loop, pr_loop_list, pr_scaling_factors, stride = {}, [], {}, []
        return pr_loop, pr_loop_list, pr_scaling_factors, stride
    ...
    def extract_pr_loop_info(self, equation_relations):
        ...
        for relation in equation_relations:
            ...
            for val_lower in relation_disassembly[1:]:
                ...
                if(val_lower == 'ox'):
                    stride_x = scaling_factors[val_lower]
                elif (val_lower == 'oy'):
                    stride_y = scaling_factors[val_lower]
        ...
        return pr_loop, pr_loop_list, pr_scaling_factors, [stride_x, stride_y]
