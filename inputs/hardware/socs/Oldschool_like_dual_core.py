from inputs.hardware.cores.Oldschool_like import (
    get_core as get_oldschool_like_core,
)
...
cores = [get_oldschool_like_core(id) for id in range(2)]  # 1 core
pooling_core = get_pooling_core(id=2)
simd_core = get_simd_core(id=3)
offchip_core_id = 4
offchip_core = get_offchip_core(id=offchip_core_id)

cores_graph = get_2d_mesh(cores, 1, 2, 32, 0, pooling_core, simd_core, offchip_core)

accelerator = Accelerator(
    "Oldschool-prototype-like-dual-core", cores_graph, offchip_core_id=offchip_core_id
)
