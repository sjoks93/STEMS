...
def get_memory_hierarchy(multiplier_array):
    ...
    w_size = 4
    reg_W_reg = MemoryInstance(name="w_rfs", size=7*7*16*w_size, r_bw=4*w_size, w_bw=4*w_size, r_cost=0.002*4*w_size, w_cost=0.002*4*w_size, area=0, r_port=1,
                             w_port=1, rw_port=0, latency=1, )
    # reg_I_reg = MemoryInstance(name="i_rfs", size=7*7*16, r_bw=7*7*16, w_bw=7*7*16, r_cost=0.002*7*7*16, w_cost=0.002*7*7*16, area=0, r_port=1,
    #                          w_port=1, rw_port=0, latency=1, )
    reg_I_reg = MemoryInstance(name="i_rfs", size=7*7*16, r_bw=4, w_bw=4, r_cost=0.002*4, w_cost=0.002*4, area=0, r_port=1,
                             w_port=1, rw_port=0, latency=1, )    
    ...
    spike_buffer_96kb = MemoryInstance(name="sbuf_96KB", size=96/2 * 8 * 1024, r_bw=7 * 7 * 16, w_bw=16 * 16,
                                                  r_cost=0.1 * 7 * 7 * 16, w_cost=0.12 *16 * 16, area=0, r_port=1, w_port=1,
                                                  rw_port=0, latency=1, min_r_granularity=64, min_w_granularity=64,)

    w_buffer_192kb = MemoryInstance(name="w_buffer_192kb", size=(192+96/2) * 8 * 1024, r_bw=64 * 4, w_bw=64 * 4,
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=reg_W_reg,
        operands=("I2",),
        port_alloc=({"fh": "w_port_1", "tl": "r_port_1", "fl": None, "th": None},),
        served_dimensions={(0, 1, 0), (0, 0, 1)},
    )
    memory_hierarchy_graph.add_memory(
        memory_instance=reg_I_reg,
        operands=("I1",),
        port_alloc=({"fh": "w_port_1", "tl": "r_port_1", "fl": None, "th": None},),
        served_dimensions={(1, 0, 0), (0, 0, 1)},
    )
    memory_hierarchy_graph.add_memory(
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=spike_buffer_96kb,
        operands=("I1", "O"),
        port_alloc=(
    ...
    memory_hierarchy_graph.add_memory(
        memory_instance=w_buffer_192kb,
        operands=("I2", "S"),
        port_alloc=(
...
def get_dataflows():
    return [{"D1": ("K", 16), "D2": ("OX", 16), "D3": ("C", 4)}]


def get_CN_node_size(factor_loops=list()):
    return [("K", 16), ("OX", 16),  ("C", 4)] + factor_loops
